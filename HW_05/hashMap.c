...
{
    struct HashMap * newMap = malloc(sizeof(HashLink*) * capacity);
...
void hashMapRemove(HashMap* map, const char* key)
{
    if(hashMapContainsKey(map, key)){
        ...
        while(cur != NULL){
            if(strcmp(cur->key, key) == 0){
                trash = cur;
                if(trash == map->table[hashIndex]){
                    if(trash->next != NULL){
                        map->table[hashIndex] = trash->next;
                    }
                    else{
